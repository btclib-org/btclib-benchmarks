...
def main() -> None:
    """Time both paths, the bindings first and then Python.

    The order is what the measurement requires: `python_arithmetic_only`
    cannot be undone within a process, so every row meant to reach the
    bindings is timed before it runs.
    """
    report_provenance()
