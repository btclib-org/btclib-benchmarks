...
on disk when it runs, and there is none in CI (issue btclib-org/.github#1070).
...
from inside `tests/` has a `.pytest_cache` there, and that one is what
the exclusion exists for. The roots come out of the exclusion patterns
...
# a bare "git" in a subprocess list is a partial executable path
...
def _source_exclude() -> tuple[str, ...]:
    ...
    return tuple(declared["tool"]["uv"]["build-backend"]["source-exclude"])
